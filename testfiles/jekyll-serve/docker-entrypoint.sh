...
if [ ! -f Gemfile ]; then
  ...
  echo ""
  echo "docker run -p 80:4000 -v \$(pwd):/site kodekloud/jekyll-serve"
  echo ""
  echo "NOTE: To create a new site, you can use the sister image kodekloud/jekyll like:"
  echo ""
  echo "docker run -v \$(pwd):/site kodekloud/jekyll new ."
  exit 1
fi
